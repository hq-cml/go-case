 ...
 * 关于时间的用法，重点是时区
 * 补充：
 * 	 UTC -- 世界统一时间（约等于已废弃的曾经的GMT），简单理解就是0度经线时间
 *   CST -- 中国标准时间，东八区时间，北京时间，UTC + 8
 * 原则：
 *   全世界，共享同一个时间戳，只不过因为时区的不同，表现出的时间是不一样的
 */
package main
...
func main() {
	...
	t1, err := time.Parse(layout, str) // 注意：因为这里不带时区，转成的是UTC时间！
	if err != nil {
		panic(err)
	}
	...
	// 时间戳是否一样？
	fmt.Println(t1.Unix(), ",", t1.In(locBeijing).Unix(), ",", t1Usa.Unix(), "可以看到，只要时间一样则时间戳一样，尽管他们根据时区格式化之后打印不一样")
	fmt.Println("-----------------------------")
